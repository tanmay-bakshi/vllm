...
class BeamSearchOnlineMixin(ABC):
    ...
    async def beam_search(
        ...
        async def generate_beam_step(
            prompt_item: EngineInput,
            request_id_item: str,
            lora_request_item: LoRARequest | None,
        ) -> list[RequestOutput]:
            stream = await self.engine_client.generate(
                prompt_item,
                sampling_params,
                request_id_item,
        ...
        for _ in range(max_tokens):
            tasks: list[asyncio.Task[list[RequestOutput]]] = []
            ...
            try:
                beam_outputs = await asyncio.gather(*tasks)
            except BaseException:
                for task in tasks:
                    task.cancel()
                await asyncio.gather(*tasks, return_exceptions=True)
                raise
            output = [result[0] for result in beam_outputs]
